...
namespace Suppliermanage
{
    class dbsupplier
    {
        ...
        public static void AddSupplier(supplier supd)
        {
            ...
            string sql = "INSERT INTO suppliermanage VALUES (NULL ,@companyname, @address, @country, @contactnumber, @email, @paymenttype, @itemtype, @description, @date )";
            ...
            cmd.Parameters.Add("@paymenttype", MySqlDbType.VarChar).Value = supd.PaymentType; ; //supd.PaymentType;
        ...
        {
            string sql = "update suppliermanage set companyname = @companyname, address = @address, country = @country, contactnumber = @contactnumber, email = @email, paymenttype = @paymenttype, itemtype = @itemtype, description = @description, date = @date where sup_id = @sup_id";
            MySqlConnection con = GetConnection();
            MySqlCommand cmd = new MySqlCommand(sql, con);
            cmd.CommandType = CommandType.Text;
            ...
            cmd.Parameters.Add("@paymenttype", MySqlDbType.VarChar).Value = supd.PaymentType;
